...
Partial Class frmGpfStmt2
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack() Then
            ...
            If s = "G" Then
                ds = New DataSet()
                sql = "select substr(tname,4,4) || '-' || substr(tname,8,2) as tname,substr(tname,4,6) tname1 from tab where tname like 'GPF20%T' and length(tname) = 10 order by tname"
                OraCn.FillData(sql, ds)
                Dim i As Integer
                For i = 0 To ds.Tables(0).Rows.Count - 1
                    Dim ds1 As New DataSet()
                    sql = "select count(*) from GPF" & ds.Tables(0).Rows(i)(1) & "T where gpfno = '" & Session("EmpGpf") & "'"
